...
class TestWebApi < Test::Unit::TestCase
  ...
  context 'The Web API' do
    ...
    setup do
      SteamCondenser::Community::WebApi.api_key = '0123456789ABCDEF0123456789ABCDEF'
    end

    should 'allow access to the API key' do
      assert_equal '0123456789ABCDEF0123456789ABCDEF', SteamCondenser::Community::WebApi.api_key
    end

    should 'allow to set the API key' do
      SteamCondenser::Community::WebApi.api_key = 'FEDCBA9876543210FEDCBA9876543210'

      assert_equal 'FEDCBA9876543210FEDCBA9876543210', SteamCondenser::Community::WebApi.api_key
    end

    should 'fail to set an invalid API key' do
      error = assert_raises SteamCondenser::Error::WebApi do
        SteamCondenser::Community::WebApi.api_key = 'test'
      end
    ...
    should 'provide easy access to parsed JSON data' do
      data = mock
      SteamCondenser::Community::WebApi.expects(:get).with(:json, 'interface', 'method', 2, { :test => 'param' }).
        returns data
      MultiJson.expects(:load).with(data, { :symbolize_keys => true }).
        returns({ :result => { :status => 1 }})

      assert_equal({ :result => { :status => 1 }}, SteamCondenser::Community::WebApi.json('interface', 'method', 2, { :test => 'param' }))
    end

    should 'provide easy access to parsed and checked JSON data' do
      SteamCondenser::Community::WebApi.expects(:json).with('interface', 'method', 2, { :test => 'param' }).
        returns({ :result => { :status => 1 }})

      assert_equal({ :status => 1 }, SteamCondenser::Community::WebApi.json!('interface', 'method', 2, { :test => 'param' }))
    end
    ...
    should 'raise an error if the checked JSON data is an error message' do
      ...
      error = assert_raises SteamCondenser::Error::WebApi do
        SteamCondenser::Community::WebApi.json! 'interface', 'method', 2, { :test => 'param' }
      end
    ...
    should 'load data from the Steam Community Web API' do
      data = mock :read => 'data'
      SteamCondenser::Community::WebApi.expects(:open).with do |url, options|
        options == { :proxy => true } &&
      ...
      assert_equal 'data', SteamCondenser::Community::WebApi.get(:json, 'interface', 'method', 2, { :test => 'param' })
    end

    should 'handle unauthorized access error when loading data' do
      io = mock :status => [401]
      http_error = OpenURI::HTTPError.new '', io
      SteamCondenser::Community::WebApi.expects(:open).raises http_error

      error = assert_raises SteamCondenser::Error::WebApi do
        SteamCondenser::Community::WebApi.get :json, 'interface', 'method', 2, { :test => 'param' }
      end
